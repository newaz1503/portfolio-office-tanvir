...
trait ResizeTrait {
    ...
    protected function resize_image( $file, $upload_path, $width, $height ) {
        ...
        $save_error = false;
        if ( in_array( strtolower( $extension ), ['.jpg', '.jpeg'] ) ) {
            imagejpeg( $new_image, $upload_path ) or ( $save_error = true );
        } elseif ( strtolower( $extension ) == '.png' ) {
            imagepng( $new_image, $upload_path ) or ( $save_error = true );
        } elseif ( strtolower( $extension ) == '.gif' ) {
            imagegif( $new_image, $upload_path ) or ( $save_error = true );
        } elseif ( strtolower( $extension ) == '.bmp' ) {
            imagewbmp( $new_image, $upload_path ) or ( $save_error = true );
        }
        if ( $save_error ) {
